            ...
            <Modal show={show} onHide={handleClose}>
                <div className="form-container">
                    <form method="POST" encType="multipart/form-data">
                        <h1 className="decorate-heading">Details of the book</h1>
                        <div className="form-input">
                            <i className="fas fa-book i-before"></i>
                            <input type="text" id="text" name="lbook" value={user.lbook} className="form-input-tag" placeholder="Enter the Book's Name" onChange={handleInputs} required />
                        </div>

                        <div className="form-input">
                            <i className="far fa-clock i-before"></i>
                            <input type="email" id="time" name="ltime" value={user.ltime} className="form-input-tag" placeholder="Enter the no of days you want to lend" onChange={handleInputs} required />
                        </div>

                        <div className="form-input">
                            <h5>Paste the front image of the book</h5>
            ...
            <div className="flex">
                {
                    test.map((ele) => {
                        ...
                        ele.map((element, ind) => {
                            ...
                            return (
                                <Card style={{ width: '18rem', height: 'fit-content' }} className="mar-style"key={ind}>
                                    <Card.Img variant="top" src={`data:image/jpeg;base64,${data}`} />
                                    <Card.Body className="second-style">
                                        <Card.Title>Book's Name: {element.lbook} </Card.Title>
                                        <Card.Title>Time period on lend(in days): {element.ltime} </Card.Title>
                                    </Card.Body>
                                </Card>  
                            );
                        })
